...
validate_legislators <- function(data, state, term) {
  ...
  zero_bill_sponsors <- data$legis_data %>%
    ...
    select("state", "term", "sponsor", "legiscan_id", "chamber", "party",
  ...
  cli_warn(glue("Found {nrow(zero_bill_sponsors)} legislators with zero ",
  ...
  review_file <- file.path(review_dir,
  ...
  if (file.exists(review_file)) {
    ...
    cli_log(glue("Found {nrow(zero_bill_sponsors_coded)} zero-LES ",
                 "legislators in file for this state/term."))

    cli_prompt(paste("If you have already reviewed and coded this list,",
                    "use cached decisions? [y/n]: ")) -> use_cached
  ...
  if (nrow(legislators_to_remove) > 0) {
    cli_log(glue("Removing {nrow(legislators_to_remove)} legislators ",
...
review_zero_les_legislators <- function(zero_bill_sponsors) {
  cli_warn(paste("Please review the following legislators with zero",
                "sponsored bills:"))
  cli_warn(paste("For each, determine if they served the FULL term or if",
                "they should be excluded"))
  cli_warn(paste("(e.g., resigned early, died, appointed mid-term",
                "replacement, etc.)"))

  for (i in 1:nrow(zero_bill_sponsors)) {
    cat("\n")
    ...
    user_input <- cli_prompt(paste("Should this legislator be REMOVED",

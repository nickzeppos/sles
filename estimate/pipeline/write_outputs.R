...
write_outputs <- function(data, state, term) {
  ...
  if (!dir.exists(outputs_dir)) {
    ...
    cli_log(glue("Created outputs directory: {outputs_dir}"))
  }

  # Write LES scores
  les_file <- file.path(outputs_dir, glue("{state}_LES_{term}.csv"))
  cli_log(glue("Writing LES scores to {basename(les_file)}..."))
  write.csv(data$les_scores, les_file, row.names = FALSE)
  cli_log(glue("  {nrow(data$les_scores)} legislators written"))
  ...
  cli_log(glue("  {nrow(data$bills)} bills written"))

  cli_log("Output files written successfully")
